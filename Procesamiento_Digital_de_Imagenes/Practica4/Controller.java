...
private void loadImage() {
    ...
    if (chooser.showOpenDialog(view) == JFileChooser.APPROVE_OPTION) {
        try {
            java.awt.image.BufferedImage img = ImageIO.read(chooser.getSelectedFile());
            model.setImage(img);
            
            // Mostrar la original inmediatamente
            view.originalCanvas.setIcon(new ImageIcon(img));
            // Limpiar la procesada anterior
            view.processedCanvas.setIcon(null);
        } catch (Exception ex) {
...
private void updateDisplay() {
    if (model.getProcessedImage() != null) {
        // Ahora actualizamos el canvas de la derecha
        view.processedCanvas.setIcon(new ImageIcon(model.getProcessedImage()));
    }
}
