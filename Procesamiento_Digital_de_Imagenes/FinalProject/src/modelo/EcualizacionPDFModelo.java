...
public class EcualizacionPDFModelo {
    ...
    public BufferedImage aplicarEcualizacion(BufferedImage img, String tipo, double alpha, int pot) {
        ...
        double sumaProb = 0;
        for (int i = 0; i < 256; i++) {
            sumaProb += (double) hist[i] / total;
            double f = 0;
            double fMin = 0;
            double fMax = 255;
            switch (tipo) {
                case "Uniforme":
                    f = fMax * sumaProb; break;
                case "Exponencial":
                    f = fMin - (1.0 / alpha) * Math.log(Math.max(0.0001, 1.0 - sumaProb)); break;
                case "Rayleigh":
                    double logTerm = Math.log(1.0 / Math.max(0.0001, 1.0 - sumaProb));
                    f = alpha * Math.sqrt(2.0 * logTerm) * 10; break;
                case "Hiperbólica":
                    double exp = 1.0 / pot;
                    f = Math.pow((Math.pow(fMax, exp) - Math.pow(fMin, exp)) * sumaProb + Math.pow(fMin, exp), pot); break;
                case "Logarítmica":
                    f = 1 + (fMax * (Math.log(1 + sumaProb) / Math.log(2))); break;
                default:
                    f = fMax * sumaProb;
            }
            mapa[i] = ImageUtils.clamp((int) Math.round(f));
        }
