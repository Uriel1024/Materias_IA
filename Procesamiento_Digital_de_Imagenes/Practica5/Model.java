...
// --- EL MÉTODO COMBINE AHORA RESPETA LOS COLORES ---
    public BufferedImage combine(String op) {
        ...
        for (int y = 0; y < h; y++) {
            for (int x = 0; x < w; x++) {
                ...
                // Extraemos los 3 canales (RGB) de la Imagen 1
                int r1 = c1.getRed();
                int g1 = c1.getGreen();
                int b1 = c1.getBlue();

                // Extraemos los 3 canales (RGB) de la Imagen 2
                int r2 = c2.getRed();
                int g2 = c2.getGreen();
                int b2 = c2.getBlue();

                // Variables para el color resultante
                int rOut = 0, gOut = 0, bOut = 0;

                switch(op) {
                    case "Suma": 
                        rOut = Math.min(255, r1 + r2); 
                        gOut = Math.min(255, g1 + g2); 
                        bOut = Math.min(255, b1 + b2); 
                        break;
                    case "Resta": 
                        rOut = Math.max(0, r1 - r2); 
                        gOut = Math.max(0, g1 - g2); 
                        bOut = Math.max(0, b1 - b2); 
                        break;
                    case "Multi": 
                        rOut = Math.min(255, (r1 * r2) / 255); 
                        gOut = Math.min(255, (g1 * g2) / 255); 
                        bOut = Math.min(255, (b1 * b2) / 255); 
                        break; 
                    case "División": 
                        rOut = (r2 == 0) ? 255 : Math.min(255, (r1 * 255) / r2); 
                        gOut = (g2 == 0) ? 255 : Math.min(255, (g1 * 255) / g2); 
                        bOut = (b2 == 0) ? 255 : Math.min(255, (b1 * 255) / b2); 
                        break; 
                    case "AND": 
                        rOut = r1 & r2; gOut = g1 & g2; bOut = b1 & b2; 
                        break;
                    case "OR":  
                        rOut = r1 | r2; gOut = g1 | g2; bOut = b1 | b2; 
                        break;
                    case "XOR": 
                        ...
                        break;
                    case "NOT": 
                        rOut = 255 - r1; gOut = 255 - g1; bOut = 255 - b1; 
                        break; 
                    case "<":  
                        rOut = (r1 < r2) ? 255 : 0; gOut = (g1 < g2) ? 255 : 0; bOut = (b1 < b2) ? 255 : 0; 
                        break;
                    case "<=": 
                        rOut = (r1 <= r2) ? 255 : 0; gOut = (g1 <= g2) ? 255 : 0; bOut = (b1 <= b2) ? 255 : 0; 
                        break;
                ...
                // Guardamos el nuevo pixel con sus 3 canales procesados independientemente
